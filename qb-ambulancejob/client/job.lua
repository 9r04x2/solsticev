    ...
    CreateThread(function()
        ...
        QBCore.Functions.GetPlayerData(function(PlayerData)
            ...
            if (not PlayerData.metadata["inlaststand"] and PlayerData.metadata["isdead"]) then
                ...
                QBCore.Functions.Notify("Ölüyken Quitledigin icin Hastanede dogdun!", "error")
...
RegisterNetEvent('hospital:client:RevivePlayer', function()
    -- Meslek ve gang kontrolü ekle
    local PlayerData = QBCore.Functions.GetPlayerData()

    -- Job kontrolü: Ambulance ya da Medic gang kontrolü
    if PlayerData.job.name ~= "ambulance" and PlayerData.gang.name ~= "medic" then
        print('31')
        return
    end

    -- Replace HasItem with TriggerCallback for item check
    QBCore.Functions.TriggerCallback("ate-base:item-kontrol", function(amount)
        if amount > 0 then
            local player, distance = GetClosestPlayer()
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                QBCore.Functions.Progressbar("hospital_revive", Lang:t('progress.revive'), 30000, false, true, {
                    ...
                    TriggerServerEvent("hospital:server:RevivePlayer", playerId, false, "dead")
                end, function() -- Cancel
                    StopAnimTask(PlayerPedId(), healAnimDict, "exit", 1.0)
            ...
            -- Notify if the medikit is not found
            QBCore.Functions.Notify('Üzerinde Medikit Yok!!', "error")
        end
    end, 'medikit') -- Pass the item name to the callback
end)
    ...
    CreateThread(function()
        ...
        while check do
            if IsControlJustPressed(0, 38) then
                ...
                elseif variable == "altkat" then
                    TriggerEvent("qb-ambulancejob:elevator_alt")
                end
            end
            Wait(1)
        end
...
RegisterNetEvent('qb-ambulancejob:stash', function(key)
        if onDuty then
...
RegisterNetEvent('qb-ambulancejob:elevator_alt', function()
    local ped = PlayerPedId()
    for k, _ in pairs(Config.Locations["altkat"]) do
        DoScreenFadeOut(500)
        while not IsScreenFadedOut() do
            Wait(10)
        end
        ...
        local coords = Config.Locations["altkat"][currentHospital]
        SetEntityCoords(ped, coords.x, coords.y, coords.z, 0, 0, 0, false)
        SetEntityHeading(ped, coords.w)
...
if Config.UseTarget then
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations["roof"]) do
            exports['qb-target']:AddBoxZone("roof" .. k, vector3(v.x, v.y, v.z), 2, 2, {
                ...
                options = {
                    ...
                    {
                        type = "client",
                        event = "qb-ambulancejob:elevator_alt",
                        icon = "fas fa-hand-point-up",
                        label = "Alt Kat",
                    },
                },
        ...
        for k, v in pairs(Config.Locations["altkat"]) do
            exports['qb-target']:AddBoxZone("alt" .. k, vector3(v.x, v.y, v.z), 1.5, 1.5, {
                name = "alt" .. k,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
