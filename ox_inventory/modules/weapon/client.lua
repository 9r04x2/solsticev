...
function Weapon.Equip(item, data)
	...
	if pdWeapons[data.name] and QBCore.Functions.GetPlayerData().job.type ~= "unit" and QBCore.Functions.GetPlayerData().job.type ~= "doj" and QBCore.Functions.GetPlayerData().job.name ~= "ambulance" and QBCore.Functions.GetPlayerData().job.name ~= "lsms"  then TriggerEvent("QBCore:Notify", "Bu silahı kullanamazsın!", "error") return end
	if client.weaponanims then
		...
		print(126, json.encode(shared.police))
...
function Weapon.Disarm(currentWeapon, noAnim)
	if exports["BakiTelli_paintball"]:inGameStatus() then return end
...
RegisterNetEvent("keremu:unarmed", function()
	Weapon.Disarm()
end)
