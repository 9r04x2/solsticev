...
RegisterNetEvent('ffa-ac', function()
	ffazone = true
end)

RegisterNetEvent('ffa-kapat', function()
	ffazone = false
end)
...
function client.openInventory(inv, data)
	if exports["progressbar"]:isDoingSomething() == false then
		if inv == "player" then
			TriggerServerEvent('envanterbussy-sv', data ,true)
		end
		if invOpen then
		...
		TriggerEvent('QBCore:Notify', "Şuanda envanteri kullanamazsın!", "error")
	end
end
		...
		if durability and durability <= 100 then
			if durability <= 0 then
				if string.match(item.name, "WEAPON_") then
					...
					TriggerServerEvent('l1-base:removeitem', item.name, 1, item.slot)
				end
							...
							if data.name == currentWeapon.ammo then
								if bekleamk == false then
									...
									if newAmmo < maxAmmo then
										...
										TriggerServerEvent('l1-base:removeitem', data.name, 1)
										bekleamk = true
										Wait(1200)
										bekleamk = false
										if cache.vehicle then
	...
	lib.addKeybind({
		...
		onPressed = function()
			...
			if vehBone == -1 then
				...
				if Vehicles.trunk and Vehicles.trunk.boneIndex then
					vehBone = GetEntityBoneIndexByName(entity, Vehicles.trunk.boneIndex[vehicleHash] or 'platelight')
				else
					vehBone = GetEntityBoneIndexByName(entity, 'platelight')
				end
			end
...
function client.closeInventory(server)
	if currentInventory ~= nil then
		if currentInventory.id ~= nil then
			TriggerServerEvent('envanterbussy-sv', currentInventory.id, false)
		end
	end
...
araniyor = false
RegisterNetEvent('envanterbussy')
AddEventHandler('envanterbussy', function(acik)
	araniyor = acik
end)
