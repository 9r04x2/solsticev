...
local function openInventory(source, invType, data, ignoreSecurityChecks)
	...
	--print(230, right, right and right.id, invType)
	if invType then 
		if invType == 'trunk' then
		--	exports['3dme']:sendME(source, 'Bagajı açar.')
		elseif invType == 'glovebox' then
			--exports['3dme']:sendME(source, 'Torpidoyu açar.')
		end
	end
...
lib.addCommand({'giveitem'}, {
	...
	if item then
        ...
        local targetPlayer = GetPlayerName(args.target)
		...
		local connect = {
			{
				["color"] = "697551",
				["title"] = "İtemver komutunu kullandı",
				["description"] = "Kullanan admin: **".. GetPlayerName(source) .."** \n Verdiği Kişi: **".. GetPlayerName(args.target) .."** \n Verdiği item: **"..item.name.." "..count.."x **",
				["footer"] = {
					["text"] = "Solstice",
					["icon_url"] = "https://example.com/png.png",
				},
			}
		}
	
		PerformHttpRequest("https://discord.com/api/webhooks/1349940188796682351/wQeC5CoD9qN6EBGbR5Vs8d66VIY_QCCjCbX5Rtm-1KqCL7dB027mtm8-YnkQznDddi2Q", function(err, text, headers) end, 'POST', json.encode({username = "Solstice", embeds = connect}), {['Content-Type'] = 'application/json'})

		source = Inventory(source) or { label = 'console', owner = 'console' }

		if server.loglevel > 0 then
			lib.logger(source.owner, 'god', ('"%s" gave %sx %s to "%s"'):format(source.label, count, item.name, inventory.label))
		end
	end
end)
...
RegisterServerEvent('l1-base:removeitem')
AddEventHandler('l1-base:removeitem', function(item, amount)
