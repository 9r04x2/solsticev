...
if not IsDuplicityVersion() then --client
    ...
    izzy.callback = function (name, ...)
        ...
        local handler = RegisterNetEvent(('izzy-events-%s:%s:%s'):format(GetCurrentResourceName(), name, id), function (...)
            p:resolve({...})
        end)

        TriggerServerEvent(('izzy-events-%s:%s'):format(GetCurrentResourceName(), name), id, ...)
    ...
    izzy.getEntityizzyrontOfEntity = function(entity, maxDistance, radius, flag)
        local maxDistance = maxDistance or 3.0
    ...
    izzy.createCallback = function (name, fn)
        RegisterNetEvent(('izzy-events-%s:%s'):format(GetCurrentResourceName(), name), function (id, ...)
            local src = source
            TriggerClientEvent(('izzy-events-%s:%s:%s'):format(GetCurrentResourceName(), name, id), src, fn(src, ...))
        end)
    end
