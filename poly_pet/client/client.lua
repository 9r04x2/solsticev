            ...
            if DoesEntityExist(pet) then
                ...
                if petHealth > 0 then
                    if petFood > 0 then
                        ...
                        petHealth = petHealth - 0.5
                        SetEntityHealth(pet, GetEntityHealth(pet) - 0.5)
                        UpdateHud()
                    end
                end
                ...
                elseif dst < 4 then
                    petMoving = false
                    if returnBall then
                        -- İşte burada artık doğrudan event yerine callback kullanıyoruz.
                        DetachEntity(ballObj)
                        DeleteEntity(ballObj)
                        QBCore.Functions.TriggerCallback('dusa_pets:GiveItem', function(success)
                            if success then
                                dusa.showNotification("Tennis ball envanterine eklendi!", "success")
                            else
                                dusa.showNotification("Tennis ball eklenemedi!", "error")
                            end
                        end, 'tennisball', 1)
                        returnBall = false
                    end
                    ...
                    if dusa.playerData.money["bank"] >= newD[i].totalPrice then
                        QBCore.Functions.TriggerCallback('dusa_pets:GiveItem', function(success)
                            if success then
                                ...
                                dusa.showNotification(Config.Notifications[Config.Locale].item_not_added or "Item eklenemedi!", 'error')
                            end
                        end, newD[i].item, newD[i].count)
                    else
                        dusa.showNotification(Config.Notifications[Config.Locale].not_enough_bank, 'error')
            ...
            if petHealth > 0 and petHealth < 100 then
                ...
                if item == 'treatmentkit' then
                    ...
                    if Config.Progressbar == 'ox_lib' then   
                        ...
                        QBCore.Functions.Progressbar("treatmentkit", 'Tedavi Ediliyor..', 10000, false, false, {
            ...
            elseif petHealth <= 0 then
                if item == 'revivekit' then
                    ...
                    if Config.Progressbar == 'ox_lib' then   
                        ...
                        QBCore.Functions.Progressbar("revive", 'Canlandırılıyor..', 20000, false, false, {
                            disableMovement = false,
                            disableCarMovement = true,
                            disableMouse = false,
                            disableCombat = true
            ...
            if petHealth > 0 and illness ~= 'healthy' and petHealth <= 95 then
                if item == 'treatmentpills' then
                    ...
                    if Config.Progressbar == 'ox_lib' then
                        ...
                        QBCore.Functions.Progressbar("pills", 'İlaç veriliyor.', 5000, false, false, {
                            disableMovement = false,
                            disableCarMovement = true,
                            disableMouse = false,
                            disableCombat = true
