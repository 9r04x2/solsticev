...
var app = new Vue({
  ...
  data: {
    ...
    wardrobe: {
      ...
      info: {
        wardrobe_p: "",
        info_h1: "BİLGİ!",
        info_p: "Evcil hayvanı giydirmek için sürükle ve bırak özelliğini kullanın",
      },
      wardrobe: [],
      slots: [{ item: [] }, { item: [] }],
  ...
  methods: {
    ...
    petsClose: function (item) {
      // Pets Close
      $.post("https://poly_pet/callPet", JSON.stringify({ pet: item }));
      // this.openInterface("petmenu", false);
      // $.post("https://poly_pet/closeUI", JSON.stringify({}));
    },
    petsGoSleep() {
      this.pets.spawnPedCode = "";
      this.pets.spawnPed = false;
    ...
    closefocus: function () {
      // Kapama Açma Animasyonu
      $.post("https://poly_pet/destroyPreview", JSON.stringify({}));
      petfocus = false;
      gsap.to(".petshop-cont", {
        opacity: 1,
        pointerEvents: "all",
    ...
    handleKeyDown(event) {
      if (event.keyCode == 27) {
        ...
        $.post("https://poly_pet/rotate", JSON.stringify({ key: "left" }));
      } else if (event.keyCode == 65) {
        $.post("https://poly_pet/rotate", JSON.stringify({ key: "right" }));
      }
    },
