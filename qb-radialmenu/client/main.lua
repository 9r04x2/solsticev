...
local function SetupJobMenu()
    ...
    if PlayerData.job.type == 'unit' then JobInteractionCheck = 'police' end
    local JobMenu = {
        id = 'jobinteractions',
        title = 'Meslek',
        icon = 'briefcase',
...
local function IsPoliceOrEMS()
    return (PlayerData.job.name == 'police' or PlayerData.job.type == 'unit' or PlayerData.job.name == 'ambulance')
end
...
RegisterCommand('radialmenu', function()
    if (not PlayerData.metadata["ishandcuffed"]) and (not IsPauseMenuActive()) and (not inRadialMenu) then
        if (not exports["s4-cuffv3"]:HandCuffed()) then
            setRadialState(true, true)
            SetCursorLocation(0.5, 0.5)
        else
            TriggerEvent("QBCore:Notify", "Bunu şu anda kullanamazsın.", "error")
        end
    end
end)
...
RegisterCommand("koltuk", function(source, args)
    ...
    if IsPedInAnyVehicle(PlayerPed, false) then
        if IsVehicleSeatFree(Arac, Koltuk) then 
            ...
            QBCore.Functions.Notify("Koltuk dolu", "error")
        end
    end
end)
...
RegisterCommand('pdepo', function()
    ...
    if QBCore.Functions.GetPlayerData().job.type == "unit" then
