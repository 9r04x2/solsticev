    ...
    if items then
        for name, data in pairs(items) do
            for _, v in pairs(data) do
                if v.slot then
                    options[#options+1] = {id = name .. v.slot, title = v.label, description = string.format('Durability: %s', v.metadata.durability .. '%'), serverEvent = 'gg_weaponrepair:startweaponrepair', args = {slot = v.slot, name = name, bench = i}}
                end
            end
        end
        lib.registerContext({id = 'repairbench', title = 'Silah Tamiri', options = options})
        lib.showContext('repairbench')
    end
end

RegisterNetEvent('gg_weaponrepair:repairitem', function(name)
    if lib.progressBar({
        ...
        TriggerServerEvent('gg_weaponrepair:fixweapon')
    end
end)
...
for i = 1, #Config.locations do
    ...
    if target then
        ...
        function bench:onEnter()
            lib.showTextUI('[E] - Açık Tamir Tezgahı', {icon = 'wrench'})
        end
