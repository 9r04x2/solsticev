...
RegisterNetEvent("0r-weaponReality:createOnClient", function(pickupHash, finalCoords, weaponName, weaponThrewAwayHash)
    ...
    local pickup = CreatePickupRotate(pickupHash, finalCoords.x, finalCoords.y, finalCoords.z, 0, 0, 0, 8, 1, 1, true, weaponThrewAwayHash)
    -- print("pickup" .. pickup)
    table.insert(savedWeaponPositions, { weapon = pickupHash, x = finalCoords.x, y = finalCoords.y, z = finalCoords.z + 1, realWeapon = weaponName, pickup = pickup})
end)
    ...
    if creationLaser then
        ...
        CreateThread(function()
            while creationLaser do
                local hit, coords = DrawLaser(Config.Lasers["Text"], {r = Config.Lasers["R"], g = Config.Lasers["G"], b = Config.Lasers["B"], a = Config.Lasers["A"]})
                ...
                if IsControlJustReleased(0, 38) then
                    ...
                    if hit then
                        ...
                        CreateThread(function()
                            PlayAnim(PlayerPedId(), "weapons@first_person@aim_rng@generic@projectile@shared@core", "throw_m_fb_stand", -8.0, 8.0, -1, 49) 
                            Wait(1000)
                            ClearPedTasks(PlayerPedId())
                        end)
                        Wait(550)
                        ...
                        -- print("Silahın yerde durduğu yer: " .. finalCoords.x .. ", " .. finalCoords.y .. ", " .. finalCoords.z)
                        DeleteEntity(prop)
                        local pickupHash = GetPickupHashFromWeapon(weaponThrewAwayHash)
                        -- local pickup = CreatePickupRotate(pickupHash, finalCoords.x, finalCoords.y, finalCoords.z, 0, 0, 0, 8, 1, 1, true, weaponThrewAwayHash)
                        ...
                        -- print(pickupHash, weaponName, finalCoords.x, finalCoords.y, finalCoords.z)
                        TriggerServerEvent('0r-weaponReality:weaponThrown', pickupHash, finalCoords, weaponName, weaponThrewAwayHash)
                        -- print("Weapon thrown: " .. weaponName .. " (" .. pickupHash .. ") at " .. finalCoords.x .. ", " .. finalCoords.y .. ", " .. finalCoords.z   )
                        -- table.insert(savedWeaponPositions, { weapon = pickupHash, x = finalCoords.x, y = finalCoords.y, z = finalCoords.z + 1, realWeapon = weaponName, pickup = pickup})
                    else
...
RegisterCommand("0r-weaponReality:ChangeWeaponRunningAnimation", function()
    ...
    Notification("" .. (keyPressed and Config.Notifications['changedanimationsTrue'].message or Config.Notifications['changedanimationsFalse'].message) .. "")
end)
