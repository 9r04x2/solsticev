...
Citizen.CreateThread(function()
    TriggerServerEvent("lariss-showid:add-id")
    while true do
        Citizen.Wait(1)
        if IsControlPressed(0, LARISS.key) or forceDraw then
            for k, v in pairs(GetNeareastPlayers()) do
                local ped = GetPlayerPed(-1)
                if IsPedArmed(ped, 4) then
                    Citizen.Wait(1)
                    QBCore.Functions.Notify("Elinde silah varken ID'leri göremezsin!")
                    Citizen.Wait(5000)
                else
...
RegisterNetEvent('lariss-showid:client:add-id')
AddEventHandler('lariss-showid:client:add-id', function(identifier, playerSource)
...
RegisterCommand(LARISS.commandName, function()
...
function GetNeareastPlayers()
    ...
    if IsPedInAnyVehicle(playerPed, false) then
        ...
        local players, _ = GetPlayersInArea(playerCoords, LARISS.drawDistance)
