...
async function start(){
    ...
    await delay(0.8)
    setInformationText('HACK PROCESS STARTS ...')
    await delay(1)
    setInformationText('System Infiltrated Were Short on Time Be Quick..')
    await delay(0.8)
    ...
    // display result
    setInformationText((result) ? 'Hack işlemleri başarılı.' : "The system did not accept your answers")
    ...
    fetch('https://fleeca-minigame/callback', {
        method: 'POST', 
    ...
    $('#submitted-reveal').textContent = (result) ? 'Good job, really.' : ((submitted == null) ?  "Time ran out," : ``)
}
