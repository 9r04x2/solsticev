...
AddEventHandler('s4-cuff:client:putInVehicle', function()
    ...
    for _, vehicle in ipairs(vehicles) do
        ...
        local dist = Vdist(coords, vehicleCoords)
    ...
    if closestVehicle then
        ...
        -- -1 şoför koltuğu olduğu için, yolcu koltuklarını kontrol ederiz
        for i = 1, maxSeats do
            if IsVehicleSeatFree(closestVehicle, i) then
                print("4: Found free seat at index " .. i)
                TaskWarpPedIntoVehicle(playerPed, closestVehicle, i)
                seatFound = true
                break
            end
        end
...
function EnumerateVehicles()
    return coroutine.wrap(function()
        ...
        repeat
            ...
            local success, result = FindNextVehicle(handle)
            vehicle = result
        until not success
   	...
   	if closestPlayer ~= -1 and closestDistance <= 2.0 then
		...
		if true then
			...
			if not cuffed then
				...
				QBCore.Functions.Notify("Kelepçeliyken bunu yapamazsın.", "error")
			end
			...
			QBCore.Functions.Notify(Config.Lang["no_one_nearby_cuff2"], "error")
		end
   	else
		QBCore.Functions.Notify(Config.Lang["no_one_nearby_cuff"], "error")
   	end 
...
AddEventHandler('s4-cuff:client:RemoveCuffs', function(otherPlayer)
	if otherPlayer then
		if cuffed == true then 
		cuffed = false
		startAnim() 
		TriggerServerEvent("keremu:addcuff", GetPlayerServerId(PlayerId()), otherPlayer)
		end
