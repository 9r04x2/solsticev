...
window.addEventListener("message", (e) => {
  ...
  if (e.data?.players) {
    ...
    if (e.data.players.length > 0) {
            ...
            <div data-id="${player.id}" class="player">
                <div class="details">
                  <div class="details-left">
                    <h2>${player.name}</h2>
                    <p id="gps_status">GPS <span style="color: #5f69d4;">${
                      player.gpsEnabled ? "açık" : "kapalı"
                    }</span></p>
                    <p>
                      Kelepçe Tarihi: <span style="font-weight: bold;">${
                        player.cuffDate
                      }</span>
                    </p>
                  </div>
      ...
      $(".bottom").append(
        `<h1 id="noone_cuffed">Kimseye Elektronik Kelepçe takılmadı</h1>`
      );
    }
  }
});
