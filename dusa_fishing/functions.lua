    ...
    lib.points.new({
        ...
        onEnter = function()
            ...
            if options then
                ...
                target.AddCircleZone(id, coords.xyz, 0.75, {
                    name = id,
                    debugPoly = false
                }, {
                    options = options
...
function Functions.Notify(message, notifyType)
    ...
    exports['ZSX_UIV2']:Notification(header, message, icon, 5000)
    
end
RegisterNetEvent('dusa_fishing:ShowNotification')
AddEventHandler('dusa_fishing:ShowNotification', Functions.Notify)

function Functions.GetInventoryIcon(item)
    local icon
    
    if config.inventory == 'ps-inventory' or config.inventory == 'qb-inventory' or config.inventory == 'qs-inventory' then
        icon = 'nui://'..config.inventory..'/html/images/'..item..'.png'
    elseif config.inventory == 'ox_inventory' then
        icon = 'nui://'..config.inventory..'/web/images/'..item..'.png'
    elseif config.inventory == 'codem-inventory' then
        icon = 'nui://'..config.inventory..'/html/itemimages/'..item..'.png'
    else
