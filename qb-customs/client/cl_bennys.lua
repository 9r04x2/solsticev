...
local function saveVehicle()
    ...
    if veh == 0 then
        print("[qb-customs] saveVehicle: Araç bulunamadı, kaydetme yapılmayacak.")
        return
    end

    local props = QBCore.Functions.GetVehicleProperties(veh)

    if not props or type(props) ~= "table" or not props.plate then
        print("[qb-customs] saveVehicle: Geçersiz araç özellikleri geldi!", props)
        return
    end

    TriggerServerEvent('qb-customs:server:updateVehicle', props)
end
...
function EnterLocation(override)
    ...
    if not canEnter then
        QBCore.Functions.Notify('Burada bir işlem yapamazsın!', 'error')
        ExitBennys()
        return
    end
    ...
    local welcomeLabel = (locationData and locationData.settings.welcomeLabel) or 'Welcome to Bennys Motorworks!'
...
RegisterNetEvent('qb-customs:client:purchaseSuccessful', function()
    ...
    QBCore.Functions.Notify('Satın alma başarılı.', 'success')
end)
...
-- Mekanik garajını açma işlevi
RegisterNetEvent('qb-customs:client:EnterCustomss', function(override)
    -- Oyuncu aracın içinde mi kontrol et
    if not IsPedInAnyVehicle(PlayerPedId(), false) then
        print("Araca binmeden mekanik garajını açamazsınız!")
        return
    end

    -- Lokasyon ayarları: Varsayılan olarak mevcut koordinatlar kullanılıyor
    local playerCoords = GetEntityCoords(PlayerPedId())
    local heading = GetEntityHeading(PlayerPedId())

    -- override verisi varsa onu kullan, yoksa otomatik olarak oyuncunun mevcut lokasyonu ayarla
    local coords = override.coords or playerCoords
    local mechanicName = override.name or "Bennys" -- Eğer isim belirtilmemişse varsayılanı kullan

    -- Garaj açma işlemi
    EnterLocation(coords, heading, mechanicName)
end)


RegisterNetEvent("applyBasicUpgrades")
AddEventHandler("applyBasicUpgrades", function()
    local playerPed = PlayerPedId()
    local vehicle = GetVehiclePedIsIn(playerPed, false)
    ...
    if vehicle ~= 0 then
        ...
        -- Fren, vites, motor ve süspansiyon modifikasyonlarını fulle
        SetVehicleMod(vehicle, 11, GetNumVehicleMods(vehicle, 11) - 1, false) -- Motor
        SetVehicleMod(vehicle, 12, GetNumVehicleMods(vehicle, 12) - 1, false) -- Frenler
        SetVehicleMod(vehicle, 13, GetNumVehicleMods(vehicle, 13) - 1, false) -- Süspansiyon
        SetVehicleMod(vehicle, 15, GetNumVehicleMods(vehicle, 15) - 1, false) -- Vites
        
        -- Turbo modifikasyonunu aç
