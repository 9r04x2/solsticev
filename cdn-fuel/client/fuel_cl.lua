...
if Config.ShowNearestGasStationOnly then
	...
	CreateThread(function()
		...
		while true do
			...
			local location = 0
			local label = "Benzin İstasyonu" -- Prevent nil just in case, set default name.
		...
		if shouldRecieveDiscount then
			...
			if discount ~= 0 then
				if discount == 100 then
					...
					discount = discount / 100
					if Config.FuelDebug then
						print("Math( Mevcut Yakıt Price: "..FuelPrice.. " - " ..FuelPrice * discount.. "<<-- FuelPrice * Discount)")
					end
					CachedFuelPrice = FuelPrice
					FuelPrice = (FuelPrice) - (FuelPrice*discount)
					if Config.FuelDebug then
	...
	if Config.Ox.Input then
		if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel and not RefuelingType == 'special' then
			if ReserveLevels < maxfuel then
				...
				fuel = lib.inputDialog('Benzin İstasyonu', {
					{ type = "input", label = 'Benzin Fiyatı', default = '$'.. FuelPrice .. ' Litre', disabled = true },
					{ type = "input", label = 'Mevcut Yakıt', default = finalfuel .. ' Litre', disabled = true },
					{ type = "input", label = 'Required Full Tank', default = maxfuel .. 'Litre', disabled = true },
					{ type = "input", label = 'Stations Available Gasoline', default = ReserveLevels, disabled = true },
					{ type = "slider", label = 'Full Depo Fiyatı: $' ..wholetankcostwithtax.. '',default = ReserveLevels, min = 0, max = ReserveLevels},
				})
				...
				fuel = lib.inputDialog('Benzin İstasyonu', {
					{ type = "input", label = 'Benzin Fiyatı', default = '$'.. FuelPrice .. ' Litre', disabled = true },
					{ type = "input", label = 'Mevcut Yakıt', default = finalfuel .. ' Litre', disabled = true },
					{ type = "input", label = 'Fullenmesi İçin Gereken', default = maxfuel, disabled = true },
					{ type = "slider", label = 'Full Depo Fiyatı: $' ..wholetankcostwithtax.. '', default = maxfuel, min = 0, max = maxfuel },
				})
		...
		if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel and not RefuelingType == 'special' then
			if ReserveLevels < maxfuel then
				...
				fuel = exports['qb-input']:ShowInput({
   					header = "Yakıt Miktarını Seç<br>Geçerli Fiyat: $" ..
    					FuelPrice .. " / Litre <br> Mevcut Yakıt: " .. finalfuel .. " Litre <br> Tam Depo Maliyeti: $" ..
   ...
   header = "Yakıt Miktarını Seç<br>Geçerli Fiyat: $" ..
            FuelPrice .. " / Litre <br> Mevcut Yakıt: " .. finalfuel .. " Litre <br> Tam Depo Maliyeti: $" ..
					...
					inputs = { {
						...
						isRequired = true,
						name = 'amount',
text = 'Tank, ' .. maxfuel .. ' Litre daha alabilir.'
					}}
				})
			end
		else
