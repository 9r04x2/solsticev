    ...
    if creationLaser then
        CreateThread(function()
            while creationLaser do
                local hit, coords = DrawLaser('SAHNEYİ YERLEŞTİRMEK İÇİN ~g~E~w~ BASIN\nSAHNEYİ DÜZENLEMEK İÇİN ~g~G~w~ BASIN', {r = 2, g = 241, b = 181, a = 200})
    ...
    if deletionLaser then
        CreateThread(function()
            while deletionLaser do
                local hit, coords = DrawLaser('BİR SAHNEYİ SİLMEK İÇİN ~r~E~w~ TUŞUNA BASIN\nİPTAL ETMEK İÇİN ~r~G~w~ TUŞUNA BASIN', {r = 255, g = 0, b = 0, a = 200})
