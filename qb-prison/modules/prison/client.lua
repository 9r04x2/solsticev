...
RegisterNetEvent("pickle_prisons:jailDialog")
AddEventHandler("pickle_prisons:jailDialog", function()
    local players = GetPlayersInArea()
    if #players == 0 then 
        return QBCore.Functions.Notify("Yakınlarda oyuncu yok!", "error") 
    end
    ...
    for _, ply in ipairs(players) do
        ...
        players_list[#players_list+1] = {
            label = _L("jail_dialog_player", GetPlayerName(ply), sid),
            value = sid
        }
    end
    ...
    print("oyuncu sayısı:", #players_list, "hapishane sayısı:", #prisons)

    local input = lib.inputDialog(_L("jail_dialog_title"), {
        { type = 'select', label = _L("jail_dialog_prisoner"), default = players_list[1].value, options = players_list, required = true },
        { type = 'select', label = _L("jail_dialog_prison"),   default = prisons[1].value,      options = prisons,      required = true },
        { type = 'number', label = _L("jail_dialog_sentence"), default = 1,                       min = 1,               required = true },
    })
