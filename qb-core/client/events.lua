...
RegisterNetEvent('QBCore:Command:GoToMarker', function()
    ...
    if not DoesBlipExist(blipMarker) then
        exports['ZSX_UIV2']:Notification("Hata", Lang:t('error.no_waypoint'), "fas fa-times-circle", 5000)
    ...
    if not found then
        ...
        exports['ZSX_UIV2']:Notification("Hata", Lang:t('error.tp_error'), "fas fa-times-circle", 5000)

    end

    -- If Z coord was found, set coords in found coords.
    SetPedCoordsKeepVehicle(ped, x, y, groundZ)
    exports['ZSX_UIV2']:Notification("Başarılı", Lang:t('success.teleported_waypoint'), "fas fa-check-circle", 5000)
...
RegisterNetEvent('QBCore:Command:SpawnVehicle', function(vehName)
    ...
    if IsPedInAnyVehicle(ped) then
        local oldVeh = GetVehiclePedIsUsing(ped)
...
RegisterNetEvent('QBCore:Notify', function(text, type, length, icon)
    local header, iconToUse
    if type == "error" then
        header = "Hata"
        iconToUse = "fas fa-times-circle"
    elseif type == "success" then
        header = "Başarılı"
        iconToUse = "fas fa-check-circle"
    ...
    exports['ZSX_UIV2']:Notification(header, text, iconToUse, length)
end)
