...
function SendAlert(yazi,tip,sure)
    ...
    if type(text) == "table" then 
        ...
        exports['poly-notify']:sendnotify(yazi, tip, sure)
    else 
...
function QBCore.Functions.Notify(text, textype, length)
    ...
    if textype == nil or textype == "" or textype == "info" then 
        textype = "primary"
    end
    ...
    if textype == "error" then
        header = "Hata"
        icon = "fas fa-times-circle"
    elseif textype == "success" then
        header = "Başarılı"
        icon = "fas fa-check-circle"
    elseif textype == "primary" then
        header = "Bilgi"
    ...
    exports['ZSX_UIV2']:Notification(header, text, icon, length)
end
...
function QBCore.Functions.GetVehiclesInArea(coords, area)
