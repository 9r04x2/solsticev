...
local function PurchaseSuccess(bool, price, item)
    ...
    if bool then
        QBCore.Functions.Progressbar('vending_item', 'Ürününüzü alın...', math.random(3000,5000), false, true, { -- Name | Label | Time | useWhileDead | canCancel
            ...
            QBCore.Functions.Notify('iptal edildi...', "error", 2000)
        end)
    else
        ClearPedTasks(PlayerPedId())
        ...
        if eatMoney <= Config.EatMoney then
            ...
            QBCore.Functions.Notify('Paranı yedi! Güzel!', 'error', 5000)
        else
            QBCore.Functions.Notify('Genellikle doğru düğmelere basmanız yardımcı olur!', 'error', 5000)
        end
    end
end
...
RegisterNetEvent("rs-vendings:client:Purchase", function(data)
    ...
    QBCore.Functions.TriggerCallback('rs-vendings:server:HasCash', function(hasCash)
        if hasCash then
            QBCore.Functions.Progressbar('vending_money', 'Para Atılıyor...', math.random(3000,5000), false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
